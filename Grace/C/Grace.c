...
#define xxxxxxxx "#include <stdio.h>%c#define FILENAME %cGrace_kid.c%c%c#define xxxxxxxx %c%s%c%c#define FT(x) int main() { FILE *file = fopen(FILENAME, %cw%c); if (file) { fprintf(file, xxxxxxxx, 10, 34, 34, 10, 34, xxxxxxxx, 34, 10, 34, 34, 10, 10, 10, 10, 10); fclose(file); } ; return 0; }%c/*%c    This program will save its source code into a file%c*/%cFT(xxxxxxxx)%c"
